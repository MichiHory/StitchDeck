...
- **Security scan** — before merging, scans all file contents for potential secrets: API keys, tokens, passwords, private keys, connection strings, and other sensitive data. If issues are found, a warning dialog shows a table of findings with a checkbox for each one. You can select which secrets to replace with random strings in the output — checked items will be substituted, unchecked ones will be kept as-is. A "select all" checkbox in the header lets you quickly toggle all findings. Source files are never modified — replacements are applied only to the merged output. You can also cancel to abort the merge entirely.
